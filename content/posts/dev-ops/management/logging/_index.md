---
title: Logging
date: 2023-10-15T12:46:00+09:00
description: 시스템이 작동할 때 시스템의 작동 상태의 기록과 보존, 이용자의 습성 조사 및 시스템 동작의 분석 등을 하기 위해 작동 중의 각종 정보를 기록해둘 필요가 있는데, 이 기록을 만드는 것
summary: 시스템이 작동할 때 시스템의 작동 상태의 기록과 보존, 이용자의 습성 조사 및 시스템 동작의 분석 등을 하기 위해 작동 중의 각종 정보를 기록해둘 필요가 있는데, 이 기록을 만드는 것
linkTitle: Logging
series:  
  - Management for Infrastructure
categories:
  - Dev Ops
  - Management for Infrastructure
tags:
  - Logging
lastmod: 2023-12-17T06:09:21.797Z
featured: true
---

![Data Logging](media/images/data-logging.png "https://www.motadata.com/blog/what-is-data-logging/")

## Reference

[VPC Flow logs를 네트워크 문제 분석에 활용할 수 있을까?](https://yozm.wishket.com/magazine/detail/1418/)  
[개선버전 - 로그수집기 fluentbit in TACO](https://devocean.sk.com/search/techBoardDetail.do?ID=164148&boardType=)  
[나는 생각한다, Go로 나는 분산 로깅을 개발한다](https://devocean.sk.com/blog/techBoardDetail.do?ID=165547&boardType=techBlog&searchData=&id=&vcode=&vcodeList=)  
[29CM 로그 수집 시스템 소개](https://medium.com/29cm/29cm-%EB%A1%9C%EA%B7%B8-%EC%88%98%EC%A7%91-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EC%86%8C%EA%B0%9C-e7955d7deec6)  
[Open Distro for Elasticsearch — 로그를 활용한 장애 탐지](https://medium.com/29cm/open-distro-for-elasticsearch-%EB%A1%9C%EA%B7%B8%EB%A5%BC-%ED%99%9C%EC%9A%A9%ED%95%9C-%EC%9E%A5%EC%95%A0-%ED%83%90%EC%A7%80-8ff60dc3e5f)  
[소프트웨어 엔지니어가 알아야 할 로그에 대한 모든 것](https://medium.com/rate-labs/%EC%86%8C%ED%94%84%ED%8A%B8%EC%9B%A8%EC%96%B4-%EC%97%94%EC%A7%80%EB%8B%88%EC%96%B4%EA%B0%80-%EC%95%8C%EC%95%84%EC%95%BC-%ED%95%A0-%EB%A1%9C%EA%B7%B8%EC%97%90-%EB%8C%80%ED%95%9C-%EB%AA%A8%EB%93%A0-%EA%B2%83-11513af8b998)
[다나와 웹 트래픽 로그 데이터 분석 시스템 도입기](https://danawalab.github.io/danawaservice/2023/12/13/Danawa-Logging-Analytics-Service.html)  
[Telegraf vs Fluentd vs Fluent Bit](https://velog.io/@thouy/Telegraf-vs-Fluentd-vs-Fluent-Bit)  
[Fluentd vs Telegraf 차이점 알아보기](https://blog.voidmainvoid.net/260)
